...
function sumOfCodes(str) {
  ...
  return str
    .split('')
    .reduce((sum, char, index) => sum + str.charCodeAt(index), 0);
}
...
function isPalindrome(str) {
  const prepStr = str
    ...
    .join('');
  for (let i = 0; i < Math.floor(prepStr.length / 2); i += 1) {
    if (prepStr.at(i).toUpperCase() !== prepStr.at(-(i + 1)).toUpperCase()) {
      return false;
    }
  }
  return true;
...
function encodeToRot13(str) {
  const encodeChar = (char) => {
    ...
    if (code >= 65 && code <= 90) {
      code += 13;
      if (code > 90) code = code - 90 + 65 - 1;
    }
    if (code >= 97 && code <= 122) {
      code += 13;
      if (code > 122) code = code - 122 + 97 - 1;
    }
